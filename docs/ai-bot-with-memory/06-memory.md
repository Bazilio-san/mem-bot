...
**`set_reply_mode`** с единственным параметром `mode` (`voice` или `text`). Исполнитель сохраняет предпочтение в базе
...
Намерение сменить тембр голоса оформлено отдельным инструментом **`set_voice_preference`**. Он принимает
