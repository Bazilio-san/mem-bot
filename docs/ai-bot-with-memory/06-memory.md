...
Логика из `src/pipeline/admin.js` выведена пользователю через три инструмента агента (function calling) в
`src/pipeline/tools.js`. Пользователь управляет своей памятью обычными фразами, а агент сам подбирает инструмент:
