...
очевидное, неуверенные догадки, секреты как обычный текст). Пользовательские реакции на сообщения ассистента считаются
обычными событиями истории: они дают кандидата памяти только когда целевое сообщение делает смысл реакции однозначным.
Если сохранять нечего — модель возвращает пустой список.
...
нет голосовой доставки, значение `voice` просто игнорирует и отвечает текстом. Например, эталонный интерактивный
чат `src/cli.js` печатает `res.answer` и поле `replyMode` не использует, поэтому при отсутствии голосового канала
ничего не ломается. Как голосовая доставка устроена в Telegram, описано в документации канала
(`docs/telegram/telegram-bot.md`), а не здесь.
