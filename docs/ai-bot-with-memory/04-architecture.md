...
`stream` включает потоковую выдачу финального текста модели по частям. Оба параметра необязательны: при их отсутствии
ядро работает тем же контуром, просто без потоковой обратной связи.

```js
export async function handleMessage({
  externalId, userMessage, domainKey = 'general', extractSync = false, onEvent = null, stream = false,
}) {
