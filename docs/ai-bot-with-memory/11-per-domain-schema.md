...
Домен — это проекция skill. Каждый каталог `skills/<name>/` с файлом `SKILL.md` задаёт доменный namespace памяти и всё
поведение домена: когда он применим (признаки классификации), какие инструкции добавить в основной ответ и в извлечение
фактов, какая закрытая схема структурированной памяти действует и какие инструменты и справочники доступны. Реестр
skills загружается в память при старте и является единым источником поведения домена.

`domain_key` остаётся стабильным ключом адресации памяти: на него ссылаются `memory_items`, `conversations`,
`secure_records`, `scheduled_tasks`, `topic_mentions`, `global_facts` и `global_knowledge`. Таблица `mem.agent_domains`
— тонкий справочник соответствия `domain_key` → числовой `domain_id`, на который ссылаются внешние ключи; она
заполняется из реестра skills и руками не редактируется.

Закрытая схема `data` под каждую сущность превращает поле `data jsonb` из «мешка произвольных ключей» в проверяемый
контракт и делает `entity_key` стабильным идентификатором, не жертвуя универсальностью таблицы `memory_items`. Схема
обязательна для предметного факта: факт с заданным `entity_type` сохраняется только если у его домена есть схема, в ней
объявлена эта сущность, и `data` проходит валидацию. Факт без `entity_type` (например свободное предпочтение профиля)
сущностью домена не является, схемой не описывается и проходит без изменений.

## Зачем

Без единого контракта поведение домена было бы размазано: признаки классификации в одном месте, prompt-добавки в коде,
схема в базе, инструменты по флагам. Skill собирает всё это в один файловый пакет, который человек читает, ревьюит и
держит в git. Добавление новой предметной области сводится к добавлению одного каталога `skills/<name>/`, а не к
согласованной правке нескольких подсистем.

Закрытая схема `data` решает отдельную задачу надёжности памяти. Без неё `data` хранил бы произвольный JSON (нельзя
гарантировать имена полей для машинной логики), а `entity_key` — произвольную строку (то `quadratic_equations`, то
`quadro`, и дедупликация рушится). Закрытая схема задаёт точные имена и типы полей и приводит ключ к словарю, поэтому
`data` становится машиночитаемым, дедупликация — надёжной, а извлечение опирается на строгий контракт.
...
## [SKILL-4] Что это даёт и какие риски

Выгоды: добавление предметной области — это один файловый пакет, а не правка нескольких подсистем; `entity_key` —
стабильный идентификатор, поэтому дедупликация и обновление вместо дублей надёжны; `data` валиден по типам, его
безопасно читать инструментами и фильтровать в SQL по конкретным полям; извлечение опирается на строгий
...
Риски: качество схемы требует ручного ревью файла skill; рост словаря `fixed_vocab` нужно мониторить по пометкам
канонизации и расширять; при смене `data_schema` ранее сохранённые факты остаются в прежней форме, поэтому история
изменения skill ведётся в системе контроля версий; канонический ключ не отменяет эмбеддинги для смыслового сходства,
когда ключи всё же разошлись.
...
- Классификация по skills и строгий контракт при извлечении — [08-prompts-and-models.md](08-prompts-and-models.md)
- Контур ответа и активный skill — [04-architecture.md](04-architecture.md)
- Справочник соответствия доменов (`mem.agent_domains`) — [05-data-schema.md](05-data-schema.md)
