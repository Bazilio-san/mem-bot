...
`secure_record_get`, `set_reply_mode`), остаются файлами в корне каталога. Реестр `index.js` собирает все модули
независимо от расположения, поэтому подпапки не влияют ни на сборку набора инструментов, ни на их имена.
...
Реестр инструментов является источником реальных действий, которые модель может обещать и выполнять. Реестр доменов
`mem.agent_domains` таким источником не является: домены помогают выбрать контекст, память и схемы, но не превращаются
в публичные возможности без соответствующего инструмента. В ответах о возможностях бот выводит действия из tool
definitions, а домены описывает только как области контекста и предметной памяти.
