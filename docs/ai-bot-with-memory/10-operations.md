...
| `global_fact_add` | Добавляю глобальный факт... | добавить глобальный факт (только администратор, флаг `GLOBAL_MEMORY_ENABLED`) |
| `global_fact_delete` | Удаляю глобальный факт... | удалить глобальный факт (только администратор, флаг `GLOBAL_MEMORY_ENABLED`) |
| `global_fact_list` | Показываю глобальные факты... | показать глобальные факты (только администратор, флаг `GLOBAL_MEMORY_ENABLED`) |
| `global_knowledge_search` | Ищу в базе знаний... | поиск по общей базе знаний (всем; флаг `GLOBAL_RAG_ENABLED`) |
| `global_knowledge_add` | Добавляю в базу знаний... | добавить текст в общую базу знаний (только администратор, флаг `GLOBAL_RAG_ENABLED`) |
| `global_knowledge_delete` | Удаляю из базы знаний... | удалить текст общей базы знаний (только администратор, флаг `GLOBAL_RAG_ENABLED`) |
| `set_reply_mode` | Настраиваю формат ответа... | сохранить пользовательскую форму ответа (флаг `VOICE_OUTPUT_ENABLED`) |
| `set_voice_preference` | Настраиваю голос ответа... | сохранить пользовательский тембр голосового ответа (флаг `VOICE_OUTPUT_ENABLED`) |
