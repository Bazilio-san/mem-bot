...
`secure_record_get`, `set_reply_mode`, `set_voice_preference`), остаются файлами в корне каталога. Реестр `index.js`
собирает все модули независимо от расположения, поэтому подпапки не влияют ни на сборку набора инструментов, ни на их
имена.
...
| `set_reply_mode` | Форма ответа: голос или текст | сохранить пользовательскую форму ответа (флаг `VOICE_OUTPUT_ENABLED`) |
| `set_voice_preference` | Тембр голосового ответа | сохранить пользовательский тембр голосового ответа (флаг `VOICE_OUTPUT_ENABLED`) |
...
- **Форма ответа.** `set_reply_mode` сохраняет `voice` или `text`, помечает `ctx.replyMode` и возвращает сохранённое
  значение.
- **Тембр голоса.** `set_voice_preference` валидирует выбор, сохраняет `voice_output_voice`, помечает
