...
(команда `NOTIFY`), а воркер подписан на этот канал выделенным соединением (команда `LISTEN`). Время до ближайшего
...
простое база и процессор почти не нагружаются: пустых опросов нет, пока нет работы. В интерактивном чате тот же
проход вызывается фоном и командой `/tick` вручную. Этот же воркер при включённых флагах выполняет контуры
проактивности — см. [09-proactivity.md](09-proactivity.md).
