...
Подаётся отдельным system-сообщением только тогда, когда пользователь спрашивает о возможностях, функциях, инструментах
или доменах бота. Блок намеренно разделяет области контекста и реальные инструменты. Домены берутся из
`mem.agent_domains` и описывают классификацию, предметную память, схемы `data`, темы и доменные глобальные факты. Они не
являются умениями, командами или обещанием действия.
...
Ответ о возможностях строится из доступных инструментов, включённых флагов и редакционной статьи RAG. Наличие домена
разрешает говорить только об области, в которой бот может держать контекст и предметную память. Формулировки вида
