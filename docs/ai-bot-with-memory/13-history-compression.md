...
|----------|---------:|-------|
| `historyCompression.hotWindow` | `8` | последние восемь сообщений не сжимаются вообще |
| `historyCompression.maxTokens` | `2000` | если холодная зона превысила этот размер, запускаем сжатие |
| `historyCompression.shrinkTokens` | `800` | после сжатия дайджест должен быть не больше этого размера |
| `historyCompression.zoneWeights` | `0.55, 0.30, 0.15` | доли бюджета дайджеста на ближнюю, среднюю и дальнюю зоны |
...
Таблица `mem.conversation_summaries` и её служебные колонки определяются в единой инициализации `001_init.sql` (см.
[05-data-schema.md](05-data-schema.md)). Набор служебных колонок идемпотентен — на уже существующей базе он
довыставляется через `ALTER TABLE ... ADD COLUMN IF NOT EXISTS ...`:
...
Ветвь `config.historyCompression` — одно из полей объекта `config`, в том же стиле, что ветви `config.companion` и
`config.proactive`. Значения по умолчанию заданы в `config/default.yaml`, а булевы флаги принимают вид `true` или
`false`:

```yaml
# config/default.yaml
historyCompression:
  enabled: true           # поджатие старой части истории диалога; по умолчанию включено
  hotWindow: 8
  maxTokens: 2000
  shrinkTokens: 800
  zoneWeights: [0.55, 0.30, 0.15]
  model: <AUX_MODEL>      # по умолчанию совпадает с config.llm.auxModel
  minCompressGain: 0.35
```

При старте приложения проверяется инвариант гистерезиса: значение `config.historyCompression.shrinkTokens` должно быть
строго меньше `config.historyCompression.maxTokens`, иначе сжатие запускалось бы практически на каждом сообщении. Полный
список настроек — в [03-quickstart.md](03-quickstart.md), общий объект конфигурации — в
...
посчитать суммарный размер сводки и непокрытого хвоста; если он превысил `config.historyCompression.maxTokens` — запустить суммаризатор,
...
- **`history-compress.js`** — `maybeCompressHistory(...)` решает по порогу, нужно ли сжимать, и при необходимости зовёт
  ...
  `config.historyCompression.zoneWeights`, передаёт `active_memory` суммаризатору и возвращает структурированный результат.
...
`config.historyCompression.enabled` равном `true`; при выключенном флаге слой пропускается. Слой проверяет
двенадцать пунктов:
...
| 3 | Размер после сжатия | `summary_token_count <= config.historyCompression.shrinkTokens` |
...
| 11 | Отключение функции | при `config.historyCompression.enabled` равном `false` в запрос идут только последние восемь сообщений |
