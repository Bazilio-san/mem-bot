...
«хочу женский голос», «выбери мужской голос». Модель вызывает ядровый инструмент `set_voice_preference`, и
...
коротко перечисляет допустимые варианты. Если тембр не выбран, используется глобальный `VOICE_OUTPUT_VOICE`.
...
OpenAI-compatible endpoint: `OPENAI_BASE_URL`, если нужен прокси, либо прямой OpenAI API, если переменная не задана
(модель `VOICE_OUTPUT_MODEL`, глобальный тембр `VOICE_OUTPUT_VOICE`, формат `VOICE_OUTPUT_FORMAT`). В сам запрос
