...
Распознавание включается переменной окружения `VOICE_INPUT_ENABLED`. При старте адаптер дополнительно проверяет,
...
игнорируется. Распознаватель выбирается переменной `VOICE_INPUT_PROVIDER` из пяти поддерживаемых вариантов;
значение по умолчанию — `groq-whisper-large-v3-turbo` (самый быстрый и дешёвый по итогам проверки):

| Значение `VOICE_INPUT_PROVIDER`   | Поставщик и модель                       | Ключ доступа           |
|-----------------------------------|------------------------------------------|------------------------|
| `groq-whisper-large-v3-turbo`     | Groq `whisper-large-v3-turbo`            | `GROQ_API_KEY`         |
...
Перед скачиванием адаптер проверяет лимиты. Максимальная поддерживаемая длительность — `VOICE_INPUT_MAX_SECONDS`
...
действует лимит размера `VOICE_INPUT_MAX_BYTES` (по умолчанию 25 МБ). Код языка-подсказки задаётся переменной
`VOICE_INPUT_LANG` (по умолчанию `ru`).
...
это же сообщение методом `editMessageText` с троттлингом — не чаще одного раза в `TELEGRAM_STREAM_EDIT_INTERVAL_MS`
(по умолчанию 900 мс) и не реже, чем накопится `TELEGRAM_STREAM_MIN_EDIT_CHARS` новых символов (по умолчанию 20).
На финале выполняется обязательное последнее редактирование полным текстом ответа.
...
финальный текст всё равно гарантированно доставляется. Формат сообщений — обычный текст без HTML/Markdown: незакрытая
разметка во время инкрементального редактирования ломала бы отображение.
...
Статусы инструментов. Когда модель вызывает инструмент, пользователь видит строку `Вызываю инструмент: <человеческое имя
инструмента>`. Человеческое имя берётся из поля `toolTitle` события и задано разработчиком рядом с самим инструментом;
аргументы, результаты, трассировки стека и внутренние идентификаторы в статус не попадают. Адаптер держит одно
...
переменной `TELEGRAM_TOOL_STATUS_ENABLED`; при этом индикатор «печатает…» сохраняется.

Включение и граница с голосом. Потоковая доставка работает, когда включены и ядро (`LLM_STREAMING_ENABLED`), и Telegram
(`TELEGRAM_STREAMING_ENABLED`). При выключенном `TELEGRAM_STREAMING_ENABLED` адаптер отправляет индикатор «печатает…» и
финальный ответ одним `sendMessage`, как при разовой доставке. Реакции (`delivery.kind = 'reaction'`) идут не через
поток: у них нет полного текста ответа, поэтому остаётся прежний путь через `deliverAgentResult`. При включённом
голосовом ответе (`VOICE_OUTPUT_ENABLED`) текст не стримится — голосовой ответ требует целого финального текста для
...
текстовые. Намерение распознаёт модель и вызывает ядровый инструмент `set_reply_mode`, который сохраняет
...
голосом ответ уходит только при включённом флаге `VOICE_OUTPUT_ENABLED`, иначе адаптер молча отвечает текстом.
...
длиннее жёсткого предела `VOICE_OUTPUT_MAX_CHARS` (значение по умолчанию и максимум — 500 символов) либо
содержит блоки кода или многострочные списки, бот строит краткое резюме вспомогательной быстрой моделью
(`VOICE_OUTPUT_SUMMARY_MODEL`) и озвучивает именно его, а полный ответ при этом дополнительно отправляет текстом,
...
формате OGG/OPUS, скрывая выбор поставщика и модели. Озвучивание идёт через ту же конечную точку `audio/speech`
OpenAI-совместимого прокси, что и остальные вызовы модели (модель `VOICE_OUTPUT_MODEL`, тембр
`VOICE_OUTPUT_VOICE`, формат `VOICE_OUTPUT_FORMAT`); отдельный ключ доступа не нужен. Прокси периодически
обрывает соединение по тайм-ауту, поэтому запрос синтеза повторяется несколько раз. Язык речи не задаётся: он
