...
Этот документ описывает конкретную реализацию телеграм-адаптера (точка входа `src/telegram/bot.js`, вспомогательные
модули — рядом в каталоге `src/telegram/`) и то, как его команды и экранные меню отображаются на программный интерфейс
ИИ-бота. Сама спецификация ИИ-бота (каталог `docs/ai-bot-with-memory/`) остаётся
