...
| HTTP-сервер | `src/server/index.js` (Express 5, порт 3001) | Хостинг REST API заметок и страницы Mini App |
...
## 13. Открытые вопросы (решить по месту, зафиксировать в коде/доке)

1. Поднимать MCP-сервер заметок в том же процессе, что и основной сервер, или отдельным —
   предпочтительно в том же (меньше движущихся частей), отдельный порт нужен лишь для MCP-транспорта.
2. Нужен ли `notes_search` LLM при наличии виджета — да: виджет для просмотра человеком,
   `notes_search` для ответов LLM по содержимому заметок («что я записывал про X?»).
3. Восстановление удалённых заметок (undo) — в MVP не делаем, `deleted_at` оставляет возможность.
4. Лимиты: тело заметки до ~20 000 символов; для эмбеддинга тела использовать первые ~8 000 символов.
