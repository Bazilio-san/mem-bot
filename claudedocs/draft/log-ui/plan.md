...
- Сервер: Express 5, точка входа `src/server/index.js`, API-роутер `src/server/admin-api.js` (префикс `/api`),
  порт `config.admin.port` (по умолчанию 3001). Слой данных переиспользуется из `src/sandbox/data.js`.
...
- В dev-режиме Vite-сервер на :5173 проксирует `/api` на :3001; в production Express отдаёт `web/dist`.
