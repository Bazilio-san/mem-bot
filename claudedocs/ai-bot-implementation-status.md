...
| `PROMPT-3` | Промпт извлечения кандидатов в память | готово | `src/pipeline/extract.js` |
...
| ID требования | Краткая формулировка | Статус | Ссылка на код / заметка |
|---------------|----------------------|--------|--------------------------|
| `DOMAIN-1` | Сквозной механизм per-domain-схем | нет | расширение поверх MVP; работает базовый вариант со свободным `data` |
| `DOMAIN-2` | Реестр схем, модуль `src/schema/`, миграция `004` | нет | таблица `mem.domain_schemas` и модуль `src/schema/` не созданы |
| `DOMAIN-3` | Выгоды и риски слоя | нет | актуально только после реализации `DOMAIN-1` и `DOMAIN-2` |
...
| Per-domain-схемы | слой не реализован, работает свободный `data` | `DOMAIN-1`…`DOMAIN-3` | `mem.domain_schemas`, модуль `src/schema/`, `ajv` |
