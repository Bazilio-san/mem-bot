...
const demo = {
  ...
  enabled: true,
  classification: { when_to_use: 'Когда пользователь просит демо', positive_signals: ['один', 'два'], negative_signals: ['нет'] },
...
await checkA('Невалидная схема домена отклоняется', async () => {
  const badDef = {
    domain_key: 'demo_skill', title: 'Демо',
    entities: [{ entity_type: 'x', entity_key: { mode: 'slug' }, data_schema: { type: 'object', additionalProperties: true } }],
  };
...
check('removeReference запрещает выход через ..', () => {
  assert.throws(() => removeReference('flight-search', '../SKILL.md', { confirm: true }), /Недопустимый путь|выходит за пределы/);
});
