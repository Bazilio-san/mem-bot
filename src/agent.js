...
import {
  ...
  getDomainId, getLastUserMessageTime, listDomains,
} from './repo.js';
...
11. Если пользователь спрашивает, что ты умеешь, что можешь, какие у тебя функции, домены или инструменты, ответь
    по CAPABILITIES_CONTEXT и доступным инструментам. Если доступен global_knowledge_search, сначала найди в базе
    знаний статью о возможностях бота и объедини её с тем, что видишь сам. Домены — это области контекста и памяти,
    а не умения; не формулируй домен как действие или возможность без соответствующего подключённого инструмента.
...
function isCapabilitiesQuestion(text) {
  ...
  return (
    ...
    /(твои|у тебя)\s+(возможности|функции|инструменты|домены|навыки)/i.test(s) ||
    /\b(capabilities|what can you do|features|tools|domains)\b/i.test(s)
  );
}
...
async function buildCapabilitiesContext(ctx, toolDefs) {
  ...
  const domainLines = domains.length
    ? domains.map((d) => `- ${d.domain_key}: ${d.title}${d.description ? ` — ${d.description}` : ''}`).join('\n')
...
Домены ниже — это области контекста, классификации и предметной памяти. Они НЕ являются умениями, командами или
обещанием действия. Не пиши "умею искать/делать X" только потому, что есть домен про X. Формулируй действие как
возможность только если оно поддержано доступным инструментом или явно описано в редакционной статье RAG.
Используй три источника:
1. Эту краткую карту областей контекста и инструментов.
...
Области контекста и памяти (не умения):
${domainLines}
