...
export async function generateSkillDraft({ description, hints = '' }) {
  const schema = {
    type: 'object',
    required: ['name', 'domain_key', 'title', 'description', 'when_to_use', 'positive_signals',
      'negative_signals', 'skill_prompt', 'fact_extraction_prompt', 'entities'],
    properties: {
      name: { type: 'string' },
      ...
      fact_extraction_prompt: { type: 'string' },
      entities: { type: 'array', items: ENTITY_SHAPE },
    },
  };
...
Верни только JSON по схеме.`;
  const draft = await chatJSON({
    model: authoringModel(), schema, schemaName: 'skill_draft', system,
    user: `Описание навыка: ${description}\n${hints ? `Дополнительно: ${hints}` : ''}`,
  });
  const { definition, issues } = buildDefinition({
    domain_key: draft.domain_key, title: draft.title, description: draft.description, entities: draft.entities,
  });
  return {
    name: draft.name,
...
export async function refineBlock({ kind, current, instruction, skillContext = {} }) {
  const what = kind === 'fact_extraction_prompt'
...
Название: ${skillContext.title || '—'}; описание: ${skillContext.description || '—'}`;
  const res = await chatJSON({
    model: authoringModel(), schema, schemaName: 'skill_block', system,
...
export async function generateDomainSchema({ domain_key, title, description = '', samples = [] }) {
  const schema = { type: 'object', required: ['entities'], properties: { entities: { type: 'array', items: ENTITY_SHAPE } } };
...
${samples.length ? `Примеры реплик пользователя:\n- ${samples.join('\n- ')}` : ''}`;
  const res = await chatJSON({ model: authoringModel(), schema, schemaName: 'domain_schema', system, user });
...
summary — краткое описание внесённой правки на русском. Верни только JSON по схеме.`;
  ...
  const res = await chatJSON({ model: authoringModel(), schema, schemaName: 'domain_schema_edit', system, user });
  const built = buildDefinition({
    domain_key: definition.domain_key, title: definition.title, description: definition.description, entities: res.entities,
  });
