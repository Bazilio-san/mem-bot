...
🟡 При извлечении паспорт и телефон распознаются как чувствительные и не сохраняются как обычный факт
(`requires_confirmation`), но автоматический диалог-подтверждение «Сохранить эти данные?» и автоматическая запись в
`secure_records` из разговора ещё не связаны — сохранение секрета сейчас вызывается явно (`saveSecureRecord`). Полная
интеграция «распознал секрет → спросил → сохранил по согласию» отнесена к доделкам (см. [12-appendix.md](12-appendix.md)).
