...
в истории не повторяются. Слой размещается в рекомендованных модулях `src/pipeline/history-context.js`,
`history-compress.js` и `token-counter.js`, подключается в контуре ответа `src/agent.js` и должен покрываться слоем
проверок `layerHistory`. По умолчанию он выключен флагом `HISTORY_COMPRESSION_ENABLED`, и при выключенном флаге
поведение полностью совпадает со старым (только последние восемь сообщений).
...
Слой наполняет таблицу `mem.conversation_summaries`, предусмотренную для этого в `001_init.sql` (см.
...
Проверки оформляются слоем `layerHistory` в `tests/run.js` (не в отдельном файле) и выполняются только при
