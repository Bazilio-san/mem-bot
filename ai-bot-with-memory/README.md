...
Статус реализации в конкретном проекте — то есть что уже готово, что сделано частично, а что ещё не сделано, со ссылками
на соответствующий код — вынесен во внешний артефакт. Это чеклист соответствия
`claudedocs/ai-bot-implementation-status.md`. Сама спецификация остаётся переносимой и не содержит сведений о каком-либо
конкретном проекте: она описывает, как система должна быть устроена, а не то, что уже сделано в одной конкретной реализации.
...
- **Узнать статус реализации в проекте.** Чеклист соответствия `claudedocs/ai-bot-implementation-status.md`. Туда
  переехала таблица упрощений и доделок: там перечислено, что готово, что сделано частично, а что ещё предстоит сделать.
