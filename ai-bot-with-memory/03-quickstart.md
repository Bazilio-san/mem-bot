...
такие домены, как `travel`, `landing_sales` и `math_tutor`, приведены лишь как иллюстративные примеры специализаций:
...
`/proactive welcome_back`); `/exit` — выход.
...
|------------|------------|--------------|
| `COMPANION_MODE` | темпоральный и тематический контекст в ответах плюс извлечение тем | должно включаться флагом, по умолчанию выключено |
...
tests/run.js                 комплексная проверка по слоям (базовый слой проверок плюс слои проактивности и поджатия истории)
