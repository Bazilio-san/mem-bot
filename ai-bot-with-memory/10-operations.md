...
🟡 Упрощение: расписания `cron` и `rrule` пока сведены к суточному шагу; точный разбор — доделка.
...
| `search_flights` | доменный инструмент поиска авиабилетов (заглушка вместо реального API) |
...
🔜 Структурированное логирование (JSON-логи с уровнями, корреляция по `conversation_id` и `user_id`) — доделка.
...
Фактический результат: **36/36 при выключенных флагах, 51/51 при всех включённых**.
...
- Доделки — [12-appendix.md](12-appendix.md)
