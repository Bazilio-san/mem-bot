-- migrations/012_update_bot_capabilities_rag.sql
-- Обновляет первую RAG-статью: убирает из неё сведения о конкретных подключённых доменах.
...
$rag$,
  tags = ARRAY['bot', 'capabilities', 'about', 'tools', 'rag'],
  updated_at = now(),
  metadata = metadata || '{"seed_update":"012"}'::jsonb
