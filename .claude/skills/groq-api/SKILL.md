...
Fan out independent requests concurrently with `asyncio.gather` instead of awaiting them one by one.
...
Stream synthesized audio bytes as they are produced — important for low-latency voice agents.
...
# Inspect what the model ran (web search, code execution):
...
Use `tenacity` to retry automatically on rate limits and transient errors.
...
Approximate limits by tier (Groq changes these — check console.groq.com for current values):
...
The response headers (`x-ratelimit-*`) report remaining quota and reset time.
