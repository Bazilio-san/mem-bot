...
// Текст запроса к LLM собирается здесь же: выбранный в журнале контекст (contextText) + вопрос
// администратора. Кнопка «Текст запроса в LLM» открывает редактор поверх диалога — там виден полный
...
// Автоматически собранный текст запроса: инструкция + выбранный контекст журнала + вопрос администратора.
const autoPrompt = computed(() => {
  const ctx = props.contextText || '(контекст не выбран)';
  return `Ты — опытный инженер по промптам и отладке LLM-приложений. Ниже — выбранные администратором данные из журнала бота с памятью (mem-bot): запросы к LLM (payload целиком, включая системный промпт и полные определения инструментов с описаниями и JSON-схемами параметров), ответы модели и события конвейера. Проанализируй их и ответь на вопрос администратора. Отвечай по-русски, по существу, с конкретными рекомендациями. Формат — Markdown.
...
<template>
  ...
  >
    ...
    <textarea v-model="question" class="ad-q" rows="6" />
